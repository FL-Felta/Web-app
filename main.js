    ...
    fs.readdir('page', (err, files)=>{ //files가 메뉴
        ...
        fs.readFile(`page/${name}`, 'utf8', (err,data)=>{
            ...
            if(name === undefined){ //루트 페이지
                name = 'LGH'
                data = '1년 뒤 수능을 본 우리 자신에게 메세지를 보내봅시다 (물론 남의 학번으로도 작성이 당연히 되지요 ㅎㅎ)'
                control = `<a href="/create">create</a>`
            }
            const html = template.HTML(name, list, `<h2>${name}의 메세지</h2><p>${data}</p>`, control)
        res.send(html)
        })
    })
})
...
app.post('/create_process',(req, res)=>{
    ...
    req.on('end', ()=>{
        ...
        fs.writeFile(`page/${title}`, description, 'utf8', (err)=>{
            res.redirect(302, `/?name=${title}`) //처리 후에 다른 곳으로 보냄
        })
    })
})
